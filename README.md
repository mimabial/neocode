...
- **Hyprland theme sync** — picks up the system theme from `~/.config/hypr/themes/theme.conf` (generated by my dotfiles' theme pipeline), watches `staterc` and `auto_theme_state.json` via `vim.uv.fs_event`, and reapplies on focus/file change. ~850 lines of dedicated theme manager.
...
The theme manager integrates with my Hyprland theme pipeline (in `~/dotfiles`).
If `~/.config/hypr/themes/theme.conf` exists, it reads:
