return {
  {
    ...
    lazy = true,
    priority = 75, -- Higher priority to ensure it loads before completion
    config = function()
      ...
      -- Make the configured lspkind globally available
      ...
      return {
        ...
        formatting = {
          format = function(entry, vim_item)
            -- Use lspkind for formatting if available
            if package.loaded["lspkind"] then
              -- Format using lspkind
              vim_item = require("lspkind").cmp_format({
                mode = "symbol_text",
                maxwidth = 50,
                ellipsis_char = "...",
                -- Use the symbol map from lspkind
                symbol_map = _G.lspkind_symbol_map or {}, -- Fallback to empty if not available
                menu = {
                  buffer = "[Buffer]",
                  nvim_lsp = "[LSP]",
                  luasnip = "[Snippet]",
