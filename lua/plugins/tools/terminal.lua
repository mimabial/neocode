  ...
  -- Enhanced terminal
  {
    ...
    config = function()
      require("iron.core").setup({
        ...
        -- table and map them yourself, and disable it completely with
    ...
    config = function()
      require("sniprun").setup({
        ...
        display = {
          ...
          "VirtualTextOk", -- "Display results as virtual text (only works for multiline outputs or single line outputs with a slow interpreter)
          -- "TempFloatingWindow",      -- "Display results in a floating window
          -- "LongTempFloatingWindow",  -- "Same as above, but only displays in the floating window if result is long
          -- "Terminal",                -- "Display results in a vertical terminal
          -- "TerminalWithCode",        -- "Display results and code history in a vertical terminal
        ...
        -- " to workaround crappy clipboard selection not being possible in the float window
        -- " forcing always_new will recompile and execute the current buffer
        always_new = false, -- ""
        ...
        -- " Modify behavior of compound interpreters
        extend_args = nil, -- " Lua table with extra arguments (like configuring imports)
        -- " for compiled langs (c,cpp,rust,haskell)
        clean = nil, -- " remove build files after run
        -- " This is mainly useful for interpreted languages like python, where
        -- " the interpreter can be fixed, e.g., ensuring a specific virtualenv
        override_executable = nil, -- " Lua table with overridden executables
      })
    end,
  },
  ...
  -- Command runner
  {
    ...
    config = function()
      require("overseer").setup({
        ...
        task_win = {
          border = "rounded",
          -- Height and with can be a number or a function that returns a number
          height = 10,
          width = 80,
          win_opts = {
            winblend = 10,
